...
static int64_t
OurInsertTuple(Relation index, Datum *values, bool *isnull, ItemPointer heap_tid, Relation heapRel)
{
	const		IvfflatTypeInfo *typeInfo = IvfflatGetTypeInfo(index);
	Datum		value;
	FmgrInfo   *normprocinfo;
	...
	return insert_lsm_index(index, vector->x, ItemPointerToInt64(heap_tid));
}
 ...
 */
bool
our_ivfflatinsert(Relation index, Datum *values, bool *isnull, ItemPointer heap_tid,
...
)
{
	...
	/* Insert tuple */
	uint64_t vid = OurInsertTuple(index, values, isnull, heap_tid, heap);
	// elog(DEBUG1, "[our_ivfflatinsert] returned from OurInsertTuple");
	/* Insert visibility tuple */
	InsertVisibilityTuple(index, vid, heap_tid, heap);
	// elog(DEBUG1, "[our_ivfflatinsert] returned from InsertVisibilityTuple");
