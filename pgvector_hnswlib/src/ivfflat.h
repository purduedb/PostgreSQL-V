...
#define INDEX_BUILD_BATCH 100000
// TODO: make the tids size dynamic
...
typedef struct IvfflatBuildState
{
	...
	/* Variables */
	VectorArray samples;
	VectorArray centers; // this is not actually used in our faiss implementation
	ListInfo   *listInfo;
	...
	int      num_tids;
	int      cap_tids;
	void *faissIndex; // add for storing the pointer to the index during the building phase
#ifdef IVFFLAT_KMEANS_DEBUG
	double		inertia;
	double	   *listSums;
	int		   *listCounts;
...
IndexBuildResult *our_ivfflatbuild(Relation heap, Relation index, IndexInfo *indexInfo);
// void		ivfflatbuildempty(Relation index);
void		our_ivfflatbuildempty(Relation index);
...
// Our function decalaration
IndexScanDesc our_ivfflatbeginscan(Relation index, int nkeys, int norderbys);
void		our_ivfflatrescan(IndexScanDesc scan, ScanKey keys, int nkeys, ScanKey orderbys, int norderbys);
bool		our_ivfflatgettuple(IndexScanDesc scan, ScanDirection dir);
void		our_ivfflatendscan(IndexScanDesc scan);
bool		our_ivfflatinsert(Relation index, Datum *values, bool *isnull, ItemPointer heap_tid, Relation heap, IndexUniqueCheck checkUnique
#if PG_VERSION_NUM >= 140000
						  ,bool indexUnchanged
#endif
						  ,IndexInfo *indexInfo
);
IndexBulkDeleteResult *our_ivfflatbulkdelete(IndexVacuumInfo *info, IndexBulkDeleteResult *stats, IndexBulkDeleteCallback callback, void *callback_state);
