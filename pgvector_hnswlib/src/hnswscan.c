 ...
 */
void
our_hnswrescan(IndexScanDesc scan, ScanKey keys, int nkeys, ScanKey orderbys, int norderbys)
{
	HnswScanOpaque so = (HnswScanOpaque) scan->opaque;
...
static void our_populateSortstate(IndexScanDesc scan, Datum queryVector, int k) {
	// elog(DEBUG1, "enter our_populateSortstate");
	HnswScanOpaque so;
	TupleTableSlot *slot;
	TopKTuples results;
	...
	so->topkTuples = search_lsm_index(scan->indexRelation, query_vector->x, k, hnsw_ef_search);
	// initialize the top K result
	so->topkTuplesIdx = 0;
	// elog(DEBUG1, "[our_populateSortstate] set the topK tuples");
}

bool
our_hnswgettuple(IndexScanDesc scan, ScanDirection dir)
{
	HnswScanOpaque so = (HnswScanOpaque) scan->opaque;
 ...
 */
void
our_hnswendscan(IndexScanDesc scan)
{
	HnswScanOpaque so = (HnswScanOpaque) scan->opaque;
