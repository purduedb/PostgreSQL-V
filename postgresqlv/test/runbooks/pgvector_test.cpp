...
public:
    ...
    std::map<std::string, double> parallel_insert(float* data, size_t start, size_t end,
        ...
        for (size_t batch_start = start; batch_start < end; batch_start += checkpoint_size) {
            ...
            // Fine-grained parallel loop - each vector is an independent task
    ...
    void execute_runbook(const std::string& runbook_file, const std::string& dataset_name,
                         ...
                         size_t batch_mode_start = 0, size_t batch_size = 0) {
        Timer overall_timer;
        ...
        size_t step_num = 0;
        bool in_batch_mode = (batch_mode_start > 0 && batch_size > 0);
        ...
        // Collect all steps first for batch processing
        struct StepInfo {
            size_t step_num;
            std::string step_key;
            YAML::Node step;
        ...
        // Execute operations - sequential or batch mode
        step_num = 0;
        for (size_t i = 0; i < all_steps.size(); i++) {
            ...
            bool should_batch = in_batch_mode && step_num >= batch_mode_start;
            
            if (should_batch) {
                // Collect steps for this batch
                std::vector<StepInfo> batch_steps;
                for (size_t j = i; j < all_steps.size() && batch_steps.size() < batch_size; j++) {
                    if (all_steps[j].step_num >= batch_mode_start) {
                        batch_steps.push_back(all_steps[j]);
                ...
                if (!batch_steps.empty()) {
                    std::cout << "\n========================================" << std::endl;
                    std::cout << "BATCH MODE: Processing " << batch_steps.size() 
                    ...
                    std::vector<size_t> step_successful_counts(batch_steps.size(), 0);
                    std::vector<size_t> step_failed_counts(batch_steps.size(), 0);
                    ...
                    step_infos.reserve(batch_steps.size());
                    for (size_t step_idx = 0; step_idx < batch_steps.size(); step_idx++) {
                        const auto& batch_step = batch_steps[step_idx];
                        
                        std::string op = batch_step.op;
                        size_t step_num = batch_step.step_num;
                        std::string step_key = batch_step.step_key;
                        size_t start, end, total_items, k = 0;
                        
                        if (batch_step.op == "insert" || batch_step.op == "delete") {
                            start = batch_step.step["start"].as<size_t>(0);
                            end = batch_step.step["end"].as<size_t>(0);
                            total_items = end - start;
                            k = 0;
                        } else if (batch_step.op == "search") {
                            start = 0;
                            end = num_queries;
                            total_items = num_queries;
                            k = batch_step.step["k"].as<size_t>(100);
                        } else {
                    ...
                    if (step_infos.empty()) {
                        std::cout << "No valid operations in batch" << std::endl;
                        continue;
                    }
                    ...
                    // Generate array: each element is a step index (0 to batch_size-1)
                    ...
                    for (size_t step_idx = 0; step_idx < step_infos.size(); step_idx++) {
                        ...
                        double throughput = successful / batch_elapsed;
                        
                        std::cout << "  [Batch] Step " << step_info.step_num << " - " 
                                  << step_info.step_key << " (" << step_info.op << "): "
                                  << successful << " successful, " << failed << " failed, "
                                  << std::fixed << std::setprecision(2) << batch_elapsed << "s, "
                                  << std::setprecision(0) << throughput << " ops/s" << std::endl;
                        
                        DetailedStats::OperationStats op_stats;
                        op_stats.name = step_info.step_key;
                        op_stats.type = step_info.op;
                        op_stats.time = batch_elapsed;
                        ...
                        if (step_info.op == "search") {
                            op_stats.recall = 0.0;  // Skip recall in batch mode
                    ...
                    std::cout << "✔ Batch completed in " << std::fixed << std::setprecision(2) 
                              << batch_elapsed << "s" << std::endl;
                }
            } else {
                ...
                if (step_info.op == "insert") {
                    ...
                    result = parallel_insert(dataset, start, end, num_threads, checkpoint_size, step_info.step_key);
                }
                else if (step_info.op == "delete") {
                    size_t start = step_info.step["start"].as<size_t>(0);
                    size_t end = step_info.step["end"].as<size_t>(0);
                    result = parallel_delete(start, end, num_threads, checkpoint_size, step_info.step_key);
                }
                else if (step_info.op == "search") {
                    if (queries != nullptr) {
                        size_t k = step_info.step["k"].as<size_t>(100);
                        result = parallel_search(dataset, queries, num_queries, k,
                                                 num_threads, step_num, runbook_name, step_info.step_key, false);
                    } else {
...
void print_usage(const char* prog_name) {
    ...
    std::cerr << "  --batch-mode-start <num>    Start step for batch mode (1-based, default: disabled)" << std::endl;
    std::cerr << "  --batch-size <size>         Number of steps per batch in batch mode (default: 0, disabled)" << std::endl;
...
int main(int argc, char** argv) {
    ...
    for (int i = 2; i < argc; i++) {
        ...
        } else if (arg == "--batch-mode-start" && i + 1 < argc) {
            batch_mode_start = (size_t)std::atoi(argv[++i]);
        } else if (arg == "--batch-size" && i + 1 < argc) {
            batch_size = (size_t)std::atoi(argv[++i]);
    ...
    if (batch_mode_start > 0 && batch_size > 0) {
        std::cout << "  Batch mode: enabled (start at step " << batch_mode_start 
...
// --batch-mode-start 101  --batch-size 3