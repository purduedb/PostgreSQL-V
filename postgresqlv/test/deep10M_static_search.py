...
def read_ivecs(file_path, expected_k=100):
    """
...
def create_table(conn, dim):
    with conn.cursor() as cur:
        ...
        cur.execute("DROP TABLE IF EXISTS deep_vectors;")
        cur.execute(f"""
            CREATE TABLE deep_vectors (
...
def insert_vectors(conn, vectors):
    with conn.cursor() as cur:
        for vec in vectors:
            cur.execute("INSERT INTO deep_vectors (vec) VALUES (%s);", (vec.tolist(),))
        conn.commit()
...
def create_index(conn, index_type, index_params):
    with conn.cursor() as cur:
        ...
        if index_type == "ivfflat":
            ...
            index_sql = sql.SQL("""
                CREATE INDEX bigann_vector_idx ON deep_vectors
...
# bigann10M HNSW w/o insertion
# python deep10M_static_search.py --do-index --do-query \
...
# bigann10M HNSW w/ insertion
# python deep10M_static_search.py --do-insert --do-index --do-query \
#   --file /ssd_root/dataset/deep/deep10M_base.fvecs \
#   --queries /ssd_root/dataset/deep/deep1B_queries.fvecs \
#   --gnd /ssd_root/dataset/deep/deep10M_groundtruth.ivecs \
#   --num 10000000 --nq 10000 --k 100 \
