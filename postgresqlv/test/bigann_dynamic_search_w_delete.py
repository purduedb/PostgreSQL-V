...
def delete_vectors(conn, delete_ids):
    with conn.cursor() as cur:
        for vec_id in delete_ids:
            cur.execute("DELETE FROM bigann_vectors WHERE id = %s;", (vec_id,))
        conn.commit()
...
def main():
    ...
    # delete_ids = {996720, 504814, 504813, 504815, 344334, 900185, 657386}
    delete_ids = random.sample(range(1, args.total_num + 1), args.total_num // 3)
    delete_vectors(conn, delete_ids)
    print("Deletion done.\n")
    
    # Trigger index bulkdelete by vacuum
    vacuum_table(conn, "bigann_vectors")
