...
void 
flush_segment_to_disk(LSMIndex lsm, PrepareFlushMeta prep)
{
    elog(DEBUG1, "[flush_segment_to_disk] segment whose start id = %u, end_id = %u", prep->start_sid, prep->end_sid);

    char file_path[MAXPGPATH];
    GetLsmDirPath(file_path, MAXPGPATH, lsm->indexRelId);
    ensure_dir_exists(file_path);

    // index file
    GetLSMIndexFilePath(file_path, sizeof(file_path), lsm->indexRelId, prep->start_sid, prep->end_sid);
    IndexBinarySetFlush(file_path, prep->index_bin);

    // bitmap file
    GetLSMBitmapFilePath(file_path, sizeof(file_path), lsm->indexRelId, prep->start_sid, prep->end_sid);
    write_segment_file(file_path, prep->bitmap_ptr, prep->bitmap_size);

    // mapping file
    GetLSMMappingFilePath(file_path, sizeof(file_path), lsm->indexRelId, prep->start_sid, prep->end_sid);
    write_segment_file(file_path, prep->map_ptr, prep->map_size);

    // metadata file
    write_lsm_segment_metadata(lsm->indexRelId, prep);
}
