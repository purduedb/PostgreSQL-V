...
def insert_vectors(conn, vectors, tablename = "bigann_vectors"):
    with conn.cursor() as cur:
        for vec in vectors:
            cur.execute(f"INSERT INTO {tablename} (embedding) VALUES (%s);", (vec.tolist(),))
        conn.commit()
...
def main():
    ...
    if args.do_query:
        ...
        ground_truth = read_ivecs(args.gnd, 10000)
        if not args.queries:
        ...
        # predicted = search_queries(conn, queries, args.k, 5, 150, args.tablename)
        # compute_recall(predicted, ground_truth, args.k)
        
        # print("Running similarity search (r5)...")
        # predicted = search_queries(conn, queries, args.k, 10, 200, args.tablename)
        # compute_recall(predicted, ground_truth, args.k)
        
        # print("Running similarity search (r6)...")
        # predicted = search_queries(conn, queries, args.k, 20, 250, args.tablename)
        # compute_recall(predicted, ground_truth, args.k)
        
        # print("Running similarity search (r7)...")
        # predicted = search_queries(conn, queries, args.k, 30, 300, args.tablename)
        # compute_recall(predicted, ground_truth, args.k)
        
        # print("Running similarity search (r8)...")
        # predicted = search_queries(conn, queries, args.k, 40, 350, args.tablename)
        # compute_recall(predicted, ground_truth, args.k)
        
        # print("Running similarity search (r9)...")
        # predicted = search_queries(conn, queries, args.k, 50, 400, args.tablename)
        # compute_recall(predicted, ground_truth, args.k)
        
        # print("Running similarity search (r10)...")
        # predicted = search_queries(conn, queries, args.k, 60, 450, args.tablename)
        # compute_recall(predicted, ground_truth, args.k)
        
        # print("Running similarity search (r11)...")
        # predicted = search_queries(conn, queries, args.k, 70, 500, args.tablename)
        # compute_recall(predicted, ground_truth, args.k)
        
        # print("Running similarity search (r12)...")
        # predicted = search_queries(conn, queries, args.k, 80, 550, args.tablename)
        # compute_recall(predicted, ground_truth, args.k)
        
        # print("Running similarity search (r12)...")
        # predicted = search_queries(conn, queries, args.k, 80, 600, args.tablename)
        # compute_recall(predicted, ground_truth, args.k)
...
#   --gnd /ssd_root/liu4127/diskann/pgdiskann_sift_10M_gt100 \
#   --num 10000000 --nq 10000 --k 100 \
#   --index-type hnsw --M 64 --ef-construction 75 \
#   --host localhost --port 543* --user liu4127 --dbname postgres

# diskann build index only
# python static_search.py --do-index \
#   --num 10000000 --nq 10000 --k 100 \
#   --index-type hnsw --M 16 --ef-construction 40 \
#   --host localhost --port 543* --user jin467 --dbname postgres --tablename pg_vectorscale_collection

# FIXME: recall calculation bug
# diskann query
# python static_search.py --do-query \
#   --queries /ssd_root/liu4127/diskann/cohere_large_10m_test.fbin \
#   --gnd /ssd_root/liu4127/diskann/diskann_cohere_large_10m_gt100 \
#   --num 10000000 --nq 1000 --k 100 \
#   --index-type hnsw --M 16 --ef-construction 40 \
#   --host localhost --port 543* --user jin467 --dbname postgres --tablename pg_vectorscale_collection
...
#   --file /ssd_root/dataset/sift/sift_learn.fvecs \
#   --index-type hnsw --M 16 --ef-construction 40 \
#   --host localhost --port 543* --user liu4127 --dbname postgres