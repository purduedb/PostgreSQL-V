    ...
    const std::vector<LabelT> &filter_labels, bool search_invocation)
{
    ...
    for (auto id : init_ids)
    {
        ...
        if (is_not_visited(id))
        {
            ...
            // Jiayi: Compute the distance of the current point and insert its neighbors to the search list
            float distance;
            uint32_t ids[] = {id};
            float distances[] = {std::numeric_limits<float>::max()};
            // Jiayi: If deletion is conducted concurrently, do we need to acquire locks here?
    ...
    while (best_L_nodes.has_unexpanded_node())
    {
        ...
        if (_dynamic_index)
        {
            ...
            for (auto id : _graph_store->get_neighbours(n))
            {
                ...
                if (is_not_visited(id))
                {
                    // Jiayi: not inserted into the `best_L_nodes` yet
...
consolidation_report Index<T, TagT, LabelT>::consolidate_deletes(const IndexWriteParameters &params)
{
...

    {
        ...
        if (_empty_slots.size() + _nd != _max_points)
        {
            ...
            diskann::cerr << err << ", #empty slots:" << _empty_slots.size() << ", _nd:" << _nd << ", _max_points" << _max_points << std::endl;
