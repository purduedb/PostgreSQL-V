...
IndexBuildResult *our_hnswbuild(Relation heap, Relation index, IndexInfo *indexInfo);
// void		hnswbuildempty(Relation index);
void		our_hnswbuildempty(Relation index);
...
// 					   ,IndexInfo *indexInfo
// );
bool		our_hnswinsert(Relation index, Datum *values, bool *isnull, ItemPointer heap_tid, Relation heap, IndexUniqueCheck checkUnique
#if PG_VERSION_NUM >= 140000
						,bool indexUnchanged
#endif
						,IndexInfo *indexInfo
...
IndexBulkDeleteResult *our_hnswbulkdelete(IndexVacuumInfo *info, IndexBulkDeleteResult *stats, IndexBulkDeleteCallback callback, void *callback_state);
...
void		our_hnswrescan(IndexScanDesc scan, ScanKey keys, int nkeys, ScanKey orderbys, int norderbys);
bool		our_hnswgettuple(IndexScanDesc scan, ScanDirection dir);
void		our_hnswendscan(IndexScanDesc scan);
