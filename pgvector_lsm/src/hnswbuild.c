...
static void
Our_InitBuildState(HnswBuildState * buildstate, Relation heap, Relation index, IndexInfo *indexInfo, ForkNumber forkNum)
{
	buildstate->heap = heap;
	buildstate->index = index;
	buildstate->indexInfo = indexInfo;
 ...
 */
static void
OurFreeBuildState(HnswBuildState * buildstate)
{
	VectorArrayFree(buildstate->vectors);
	pfree(buildstate->tids);
	FaissIndexFree(buildstate->hnswIndex);
 ...
 */
static void
Our_AddVector(Datum *values, ItemPointer tid, HnswBuildState * buildstate)
{
	const		HnswTypeInfo *typeInfo = buildstate->typeInfo;
	...
	{
		elog(ERROR, "[Our_AddVector] An error occurs when building the HNSW index");
	}

	// batch add vectors to the IVFFlat index
	if (vectors->length == targvectors)
	{
		FaissHnswIndexCreate(buildstate->hnswIndex, buildstate->lowest_vid, vectors);
...
ScanAllRows(HnswBuildState *buildstate)
{
	...
	{
		FaissHnswIndexCreate(buildstate->hnswIndex, buildstate->lowest_vid, buildstate->vectors);
		buildstate->lowest_vid += buildstate->vectors->length;
		elog(DEBUG1, "[Our_AddVector] return from FaissHnswIndexCreate, updated lowest_vid = %d", buildstate->lowest_vid);
		   ...
		   HnswBuildState * buildstate, ForkNumber forkNum)
{
	...
	Our_InitBuildState(buildstate, heap, index, indexInfo, forkNum);
	elog(DEBUG1, "return from Our_InitBuildState");
	...
	FaissHnswIndexInit(index, buildstate->dimensions, buildstate->m, buildstate->efConstruction, &(buildstate->hnswIndex));
	...
	int status = build_lsm_index(HNSW, relId, buildstate->hnswIndex, tids, dim, elem_size, lowest_vid, highest_vid);

	// write visibility tuples
	CreateVisibilityMetaPage(index, forkNum);
	InsertVisibilityTuples(forkNum, buildstate->index, buildstate->num_tids, buildstate->vitupdesc, buildstate->tids);

	MemoryContextSwitchTo(oldCtx);
    MemoryContextDelete(hnswBuildCtx);

	OurFreeBuildState(buildstate);
	elog(DEBUG1, "[OurBuildIndex] freed buidstate");
}
 ...
 */
IndexBuildResult *
our_hnswbuild(Relation heap, Relation index, IndexInfo *indexInfo)
{
 ...
 */
void
our_hnswbuildempty(Relation index)
{
