...
static void
lsm_index_buffer_shmem_startup(void)
{
...
_PG_init(void)
{
	...
	RequestAddinShmemSpace(40000000);

	// initialize index storage metadata (We rely on lazy initialization)
	RequestNamedLWLockTranche("vector_storage_metadata", 1);

	RequestNamedLWLockTranche(LSM_LOCK_TRANCHE_NAME, INDEX_BUF_SIZE);
    LWLockRegisterTranche(LSM_TRANCHE_ID, LSM_LOCK_TRANCHE_NAME);
	elog(DEBUG1, "[_PG_init] registered the tranche lsm_index_lock");

	// initialize the LSM index memory buffer
	prev_shmem_startup_hook = shmem_startup_hook;
    shmem_startup_hook = lsm_index_buffer_shmem_startup;
	...
	snprintf(worker.bgw_name, BGW_MAXLEN, "LSMIndexFlusher");
	snprintf(worker.bgw_library_name, BGW_MAXLEN, "vector.so"); // no .so
	snprintf(worker.bgw_function_name, BGW_MAXLEN, "lsm_index_bgworker_main"); // entry function
