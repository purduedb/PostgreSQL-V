...
// hnswlib
HierarchicalNSW<float> *hnswlib_index;
std::string index_path = "/ssd_root/liu4127/pgvector_hnswlib_index";
// std::string index_path = "/ssd_root/liu4127/deep1b_10m_thr_32_ef_40_M_16.bin";
L2Space *l2space;
...
FaissHnswIndexInit(Relation index, int dimension, int M, int efConstruction, void** hnsw_index_ptr)
{
    ...
    l2space = new L2Space(dimension);
    // TODO: capacity
    HierarchicalNSW<float> *appr_alg = new HierarchicalNSW<float>(l2space, 10000000, M, efConstruction);
    // Return pointer
    *hnsw_index_ptr = static_cast<void*>(appr_alg);
    return 0;
}
...
{
    elog(DEBUG1, "enter FaissHnswIndexCreate(hnswlib), lowest_vid = %d, vector_num = %d", lowest_vid, vectors->length);
    
    // omp_set_num_threads(1);

    HierarchicalNSW<float> *appr_alg = static_cast<hnswlib::HierarchicalNSW<float>*>(hnsw_index_ptr);
    ...
    for (int64_t i = 0; i < vectors->length; ++i)
    {
        ...
        // Copy into flat_data at the correct offset
        appr_alg->addPoint(vec_ptr->x, (labeltype)lowest_vid + i);
    }   
...
FaissHnswIndexStore(void* hnsw_index_ptr, size_t *index_size, dsm_segment **return_index_seg)
{
    ...
    hnswlib_index = appr_alg;
    // ---------- hnswlib ------------
    
    // FIXME: remove faiss related operations
    // faiss::Index* index = static_cast<faiss::Index*>(hnsw_index_ptr);

    std::string index_data = "none";
...
FaissHnswIndexSearch(void *serialized_index, uint32_t index_hdl, size_t index_size, const float *query_vector, int top_k, int efSearch)
{
    // --------hnswlib--------
    int dim = 96;
    if (hnswlib_index == nullptr)
    {
        l2space = new L2Space(dim);
        hnswlib_index = new HierarchicalNSW<float>(l2space, index_path, false);
    }

    hnswlib_index->setEf(efSearch);
    // uint8_t query_vector_b[dim];
    // for(int i = 0; i < dim; i++) {
    //     query_vector_b[i] = (uint8_t) query_vector[i];
    // }
    std::priority_queue<std::pair<float, labeltype >> hnswlib_result = hnswlib_index->searchKnn(query_vector, top_k);
    ...
    for (int i = 0; i < top_k; i++) {
        if (!hnswlib_result.empty())
        {
            result->vids[i] = (int64_t) hnswlib_result.top().second;
            result->distances[i] = (float) hnswlib_result.top().first;
            hnswlib_result.pop();
        }
        else
        {
            elog(LOG, "[HnswlibIndexSearch] less than topk results are returned from the hnswlib search");
